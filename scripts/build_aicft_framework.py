...
def ws1_mappings() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        for bid, (lo, strength, rationale) in items.items()
    }
...
def ws_dt_mappings() -> dict[str, list[dict[str, Any]]]:
    ...
    items["DT_E_TP"] = [comp(
        "LO3.1.1", "strong", "direct", "Acquire",
        "Defines true positive (TP) — confusion-matrix literacy (Section E).",
    )]
    items["DT_E_FP"] = [comp(
        "LO3.1.1", "strong", "direct", "Acquire",
        "Defines false positive (FP) — confusion-matrix literacy (Section E).",
    )]
    items["DT_E_sensitivity_formula"] = [comp(
        "LO3.1.1", "strong", "direct", "Acquire",
        "Recalls sensitivity formula TP/(TP+FN) as text.",
    )]
    items["DT_E_MCR_formula"] = [comp(
        "LO3.1.1", "strong", "direct", "Acquire",
        "Recalls misclassification-rate formula (FP+FN)/N as text.",
    )]
    ...
    items["DT_G_overfitting"] = [comp(
        "LO3.1.1", "moderate", "direct", "Acquire",
        "Defines overfitting as poor generalization beyond training data.",
    )]
    items["DT_G_DT_definition"] = [comp(
        "LO3.1.2", "strong", "direct", "Acquire",
        "Defines decision tree as a data-driven rule model with tree structure.",
    )]
    items["DT_G_Q1"] = [comp(
