...
class TestCalibrationIntegration(unittest.TestCase):
    """Real scan from Felicity calibration bundle (Worksheets1-10)."""

    def setUp(self) -> None:
        cal = calibration_bundle_dir("Worksheets1-10.pdf")
        self.page = (cal / "page_2.jpg") if cal else None
        if self.page is None or not self.page.exists():
...
class TestWS5Calibration(unittest.TestCase):
    def setUp(self) -> None:
        cal = calibration_bundle_dir("Worksheets1-10.pdf")
        self.page = (cal / "page_6.jpg") if cal else None
        if self.page is None or not self.page.exists():
