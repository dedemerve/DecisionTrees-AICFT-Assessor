...
ENVELOPE_FIELDS = frozenset({"schema_version", "stage", "student_id", "worksheet", "updated_at"})
...
def _validate_envelope(data: dict, stage: str, prefix: str, errors: list[str]) -> None:
    if data.get("schema_version") != ARTIFACT_SCHEMA_VERSION:
        _err(errors, f"{prefix}: schema_version must be {ARTIFACT_SCHEMA_VERSION!r}")
...
def validate_summary_data(data: dict, prefix: str, errors: list[str]) -> None:
    _validate_envelope(data, "summary", prefix, errors)
    payload = artifact_payload(data)
    for field in ("max_score", "review_items"):
        if field not in payload:
            _err(errors, f"{prefix}: missing {field}")
    if "learning_objects" in payload:
        _err(errors, f"{prefix}: learning_objects belong in portfolio, not worksheet summary")
...
def validate_student_outputs(student_id: str, errors: list[str]) -> None:
    ...
    for ws in worksheets:
        ...
        summary = load_artifact(student_id, ws, "summary")
        if summary:
            validate_summary_data(summary, f"{prefix}/summary.json", errors)
