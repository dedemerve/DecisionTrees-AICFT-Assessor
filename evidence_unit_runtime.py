...
def build_evidence_unit(
    *,
    ...
    if meta.ocr_confidence is not None:
        confidence["ocr"] = meta.ocr_confidence
    ...
    return {
        ...
        "content": norm.normalized_content,
        "confidence": confidence,
    }


def _worksheet_unit_view(unit: dict[str, Any]) -> dict[str, Any]:
    """Per-worksheet file: document carries student/worksheet context."""
    return {k: v for k, v in unit.items() if k not in ("student_id", "worksheet_id")}
...
def build_evidence_units_from_worksheet(
    ...
    *,
    ...
    return {
        "student_id": student_id,
        "worksheet_id": worksheet_id,
        "evidence_units": [_worksheet_unit_view(u) for u in units],
    }
