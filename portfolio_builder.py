...
Reads all students/<id>/<WS>/evidence.json (+ summaries for review flags) and writes
...
def _collect_worksheet_evidence(
    ...
    for ws in list_worksheets(student_id):
        ...
        review_items = set(summary.get("review_items") or [])
...
def detect_data_gaps(
    ...
    for ws in list_worksheets(student_id):
        ...
        for item_id in summary.get("review_items") or []:
...
def build_portfolio(student_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "methodology": {
            "assessment_framework_version": FRAMEWORK_SCHEMA_VERSION,
            "portfolio_builder_version": PORTFOLIO_BUILDER_VERSION,
            "aggregation_note": (
