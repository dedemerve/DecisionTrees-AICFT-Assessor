...
def save_portfolio(
    ...
    out.pop("assessment_framework_version", None)
    out.pop("portfolio_builder_version", None)
...
def build_summary_from_scoring(
    ...
    *,
    ...
    """In-memory worksheet scorecard view from scoring payload (not persisted)."""
...
def load_worksheet_summary_view(
    student_id: str,
    worksheet: str,
    base_dir: Path | None = None,
) -> dict[str, Any]:
    """Scorecard view derived from scoring.json; validation blocked overrides when set."""
    scoring = artifact_payload(load_artifact(student_id, worksheet, "scoring", base_dir))
    if not scoring:
        return {}
    view = build_summary_from_scoring(scoring, worksheet=worksheet)
    validation = artifact_payload(load_artifact(student_id, worksheet, "validation", base_dir))
...
def save_scoring_bundle(
    ...
    *,
    ...
    """Write scoring.json and derived evidence artifact."""
    meta = {"stage", "student_id", "worksheet", "learning_objects"}
...
def _strip_meta(section: dict[str, Any]) -> dict[str, Any]:
    skip = {"worksheet", "student_id", "stage"}
