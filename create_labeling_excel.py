...
WS_TASKS = {
    ...
    # Worksheet labels: Gorev 1, Gorev 2, Alistirma 3, Gorev 4
    "WS4 - Best Threshold": [
        ("Task 1\nCircle 4 misclassified",
         "Task 1: Circles all 4 incorrectly classified food cards at Leo's threshold",
         "Acquire"),
        ("Task 2\nBetter threshold position",
         "Task 2: Places threshold between two adjacent distinct fat values; written justification present",
         "Deepen"),
        ("Exercise 3\nPia correct?",
         "Exercise 3: Explains why equal-valued items cannot be split (Pia is correct; equal values cannot be separated)",
         "Deepen"),
        ("Task 4\nBest energy threshold",
         "Task 4: Sorts cards, fills table, writes best energy threshold in final blank",
         "Deepen"),
    ...
    # Worksheet has 3 identical variable blocks (no numbered labels) + a final choice blank
    "WS5 - Try Thresholds": [
        ("Block 1\nVariable + threshold",
         "Block 1: Names a valid dataset variable and fills in threshold value(s)",
         "Acquire"),
        ("Block 1\nCount consistency",
         "Block 1: Recommended + not-recommended counts sum correctly to total number of cards",
         "Deepen"),
        ("Block 2\nVariable + threshold",
         "Block 2: Names a different valid variable and fills in threshold value(s)",
         "Acquire"),
        ("Block 2\nCount consistency",
         "Block 2: Counts sum correctly",
         "Deepen"),
        ("Block 3\nVariable + threshold",
         "Block 3: Names a third valid variable and fills in threshold value(s)",
         "Acquire"),
        ("Block 3\nCount consistency",
         "Block 3: Counts sum correctly",
         "Deepen"),
        ("Final blank\nChosen threshold",
         "Final line: states which variable and threshold they selected and records it",
         "Deepen"),
        ("Overall WS5", "Overall AI-CFT level for WS5 as a whole", ""),
        ("Notes WS5", "Free notes", ""),
    ],

    # ── WS6: Documentation of the Decision Tree ────────────────────────────────
    # Diagram blanks — no numbered tasks
    "WS6 - Draw Tree": [
        ("Root node\nVariable",
         "Root node blank: names a variable for the first split",
         "Deepen"),
        ("Branch labels\n<= and >",
         "Branch labels: writes <= and > with threshold values on both branches",
         "Acquire"),
        ("Left leaf\nClassification",
         "Left leaf: correct classification label (recommended / not recommended)",
         "Acquire"),
        ("Left leaf\nCounts",
         "Left leaf: correct counts in brackets [recommended] [not-recommended]",
         "Deepen"),
        ("Right leaf\nClassification",
         "Right leaf: correct classification label",
         "Acquire"),
        ("Right leaf\nCounts",
         "Right leaf: correct counts in brackets",
         "Deepen"),
        ("2nd level node\nVariable (if present)",
         "Second-level node: names a valid variable for further split (score if present)",
         "Deepen"),
...
def build_instructions(wb):
    ...
    levels = [
        ...
        ("Deepen",   CLR["deepen"],
         ...
         "WS3 own threshold; WS4 Exercise 3 Pia; WS_DT A4 data justification"),
        ("Create",   CLR["create"],
