...
PIPELINE_STAGES = ("extraction", "validation", "scoring", "evidence")
...
WS_DT_DETERMINISTIC_ITEM_IDS: frozenset[str] = frozenset({
    ...
    "DT_E_sensitivity_formula", "DT_E_MCR_formula",
    "DT_E_sensitivity", "DT_E_MCR",
})


def is_interpretive_rubric_item(
    item: dict[str, Any],
    ...
    *,
    ...
    WS_DT text/reflection items are interpretive by default; EMIT and metric fields are not.
    """
    mode = item.get("scoring_mode")
    if mode == "interpretive":
        return True
    if mode in ("deterministic", "emit_record"):
        return False
    if item_id and item_id in WS_DT_DETERMINISTIC_ITEM_IDS:
        return False
    if item.get("check") in ("emit_consistency", "formula", "numeric_consistency"):
        return False
    if item.get("evaluation") in ("emit_output", "numeric_value"):
        return False
    return (rubric or {}).get("worksheet") == "WS_DT"
...
ITEM_IDS_DT: list[str] = [
    ...
    "DT_E_TP", "DT_E_FP", "DT_E_sensitivity_formula", "DT_E_MCR_formula",
...
def validate_rubric_v3(rubric: dict[str, Any], source: str = "") -> list[str]:
    ...
    for item_id, item in items.items():
        ...
        if check == "unordered_token_set":
            groups = item.get("token_groups")
            if not isinstance(groups, list) or not groups:
                errors.append(f"{prefix}{item_id}: unordered_token_set requires non-empty token_groups[]")
            else:
                for i, group in enumerate(groups):
                    if isinstance(group, dict):
                        if not group.get("aliases") and not group.get("id"):
                            errors.append(
                                f"{prefix}{item_id}: token_groups[{i}] needs id or aliases"
                            )
                    elif not isinstance(group, list) or not group:
                        errors.append(f"{prefix}{item_id}: token_groups[{i}] must be object or alias list")
            if item.get("need_tokens") is None:
                errors.append(f"{prefix}{item_id}: unordered_token_set requires need_tokens")

        if check == "any_of_tokens":
            has_aliases = bool(item.get("accepted_aliases"))
            has_sets = bool(item.get("accept_sets"))
            if not has_aliases and not has_sets:
                errors.append(
                    f"{prefix}{item_id}: any_of_tokens requires accepted_aliases or accept_sets"
                )
            if has_sets:
                eq = rubric.get("equivalence_sets")
                if not isinstance(eq, dict):
                    errors.append(f"{prefix}{item_id}: accept_sets requires rubric equivalence_sets")
                else:
                    for key in item["accept_sets"]:
                        if key not in eq:
                            errors.append(
...
def rubric_to_assessor_criteria(
    item_id: str,
    item: dict[str, Any],
    rubric: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    if item.get("example_answer"):
        if is_interpretive_rubric_item(item, rubric, item_id=item_id):
            parts.append(
                "Illustrative example only (many valid answers): "
                + item["example_answer"]
            )
        else:
    ...
    if evaluation in ("true_false",):
        ...
        for group in item.get("token_groups") or []:
            if isinstance(group, dict):
                ...
                full.append(f"  - {label}: accepts {', '.join(aliases) if aliases else label}")
            else:
        ...
        aliases = resolve_accepted_aliases(item, rubric or {})
        full.append(
            "Any one accepted term earns full credit (equivalence pairs on this worksheet)."
        )
        for alias in aliases:
            full.append(f"  - {alias}")
        zero.append("None of the accepted equivalent terms present")

    elif is_interpretive_rubric_item(item, rubric, item_id=item_id):
        policy = (rubric or {}).get("scoring_policy") or {}
        full.append(
            policy.get(
                "component_scoring",
                "No single canonical answer. Score on rubric components and the student's "
                "own CODAP reasoning.",
            )
        )
        full.append(
            "Do NOT downgrade because the student's variable, threshold, or metric values "
            "differ from the illustrative example."
        )
        if item.get("components"):
            ...
            full.append(f"Full credit when {need} of {len(required)} required components are met:")
            for c in required:
                full.append(f"  - {c.get('id', 'component')}: {c.get('idea', '')}")
            partial.append(
                f"Partial when {partial_on} of {need} components are present, "
...
def get_assessor_rubric(item_id: str, worksheet: str | None = None) -> dict[str, Any]:
    ...
    return rubric_to_assessor_criteria(item_id, rubric_item(ws, item_id), load_rubric(ws))
...
PDF_STUDENT_ORDER_PATH = REPO_ROOT / "calibration" / "pdf_student_order.json"


@lru_cache(maxsize=1)
def load_pdf_student_order_doc() -> dict[str, Any]:
    if not PDF_STUDENT_ORDER_PATH.exists():
        return {}
    return json.loads(PDF_STUDENT_ORDER_PATH.read_text(encoding="utf-8"))


def dry_run_folder_name(pdf_name: str, slot_index: int) -> str:
    """
    Folder label for dry_run image output (0-based slot index).

    Uses calibration/pdf_student_order.json:
      1. slot_overrides (1-based slot number -> pseudonym)
      2. pdfs[pdf_name] ordered list
      3. fallback slot_{NN}
    """
    doc = load_pdf_student_order_doc()
    slot_1based = str(slot_index + 1)
    overrides = doc.get("slot_overrides", {}).get(pdf_name, {})
    if slot_1based in overrides:
        return overrides[slot_1based]

    order = doc.get("pdfs", {}).get(pdf_name)
    if isinstance(order, list) and slot_index < len(order):
        name = order[slot_index]
...
def calibration_bundle_dir(pdf_name: str = "Worksheets1-10.pdf") -> Path | None:
    """Human-verified scan bundle used for layout/HTR calibration (Felicity)."""
    doc = load_pdf_student_order_doc()
    cal = doc.get("calibration_bundle", {})
    student_key = cal.get("student_key", "Felicity")
    cal_pdf = cal.get("pdf", pdf_name)
    pdf_stem = pdf_to_images_stem(cal_pdf)
    direct = OCR_IMAGES_DIR / pdf_stem / student_key
    if direct.is_dir() and any(direct.glob("page_*.jpg")):
        return direct

    legacy_name = cal.get("legacy_dir", "_slot31_check")
    legacy = OCR_IMAGES_DIR / legacy_name
    if legacy.is_dir() and any(legacy.glob("page_*.jpg")):
        return legacy
...
def slot_dir_for_student(student_key: str, pdf_name: str = "Worksheets1-10.pdf") -> Path | None:
    ...
    Checks, in order:
      ...
      4. Calibration bundle (Felicity) for Sample_Student
      5. Legacy slot_{NN} folders (backward compatible)
    """
    ...
    if marker.exists():
        try:
            ...
            named = dry_run_folder_name(pdf_name, slot_num - 1)
            named_dir = base / named
            if named_dir.is_dir():
                return named_dir
    ...
    if student_key == "Sample_Student":
        cal = calibration_bundle_dir(pdf_name)
        if cal is not None:
            return cal
